/**
 * useLibraryUrl.ts
 * Custom hook for managing library page URL state (search, filters, sorting, pagination).
 * Provides synchronized URL parameters with debounced search updates.
 * Enables shareable URLs and browser back/forward navigation for library state.
 */
...
/**
 * useLibraryUrl Hook
 * Manages library page state via URL search parameters.
 ...
 *
 * @returns Object with current params (query, genre, platform, status, sortBy, order, page) and setters
 */
...
export const useLibraryUrl = () => {
  ...
  // Debounced search setter (300ms like Catalog)
...
// Exported to LibraryPage for URL-based state management
